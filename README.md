...
Material from a sample class for the Maestría en Administración de Riesgos, Universidad Anáhuac.
Everything is available in **English (`_ENG`)** and **Spanish (`_ESP`)**; the class was delivered
in Spanish.
...
1. **Simulating the future** — 10,000 paths of the TIIE de Fondeo short rate under the risk-neutral
   measure with a calibrated two-factor Gaussian model (G2++). Exact scheme, no discretisation
   error.
