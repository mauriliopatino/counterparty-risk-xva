...
## 1 · What is your counterparty's promise to pay worth?

### CVA and DVA with simulation and machine learning

Material from a sample class for the Maestría en Riesgo, Universidad Anáhuac. Everything is
available in **English (`_ENG`)** and **Spanish (`_ESP`)**; the class is delivered in Spanish.

| Item | English | Español |
|---|---|---|
| Slides (23 frames, ~35 min) | [`slides_ENG.pdf`](slides_ENG.pdf) | [`slides_ESP.pdf`](slides_ESP.pdf) |
| Live demo notebook | [`cva_dva_demo_ENG.ipynb`](cva_dva_demo_ENG.ipynb) | [`cva_dva_demo_ESP.ipynb`](cva_dva_demo_ESP.ipynb) |
| Lecture notes (15 to 16 pages) | [`lecture_notes_ENG.pdf`](lecture_notes_ENG.pdf) | [`lecture_notes_ESP.pdf`](lecture_notes_ESP.pdf) |

Run the demo in the browser, nothing to install:

[![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/mauriliopatino/counterparty-risk-xva/blob/main/cva_dva_demo_ENG.ipynb) English
&nbsp;&nbsp;
[![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/mauriliopatino/counterparty-risk-xva/blob/main/cva_dva_demo_ESP.ipynb) Español

### What the demo covers

1. **Simulating the future:** 10,000 paths of the TIIE de Fondeo short rate under the risk-neutral
   measure with a calibrated two-factor Gaussian model (G2++). Each factor is simulated exactly,
...
5. **The price:** CVA, DVA and a simplified bilateral adjustment (it does not model which party
   defaults first), including the break-even point where the adjustment changes sign.
6. **One swap, six counterparties:** with illustrative spreads, the price of risk changes by a
   factor of almost seven.
7. **Monte Carlo convergence:** why the engine is expensive; the error decays as one over the
   square root of the number of paths.
8. **The machine learning surrogate,** trained on labels generated by the engine itself.
9. **The race:** the full engine, the engine reusing its stored cube, and the surrogate, one query
   at a time and in batch; then a real-time pre-deal quote, reconciled against the engine.

The notebook runs end to end in well under a minute on a standard laptop. `xgboost` is optional: if
it is not installed, the notebook falls back automatically to scikit-learn's
`HistGradientBoostingRegressor`.

**Simplifications made so the demo runs live:** a flat curve, quarterly payment dates,
expected exposure discounted with the market curve, exposure independent of default, and illustrative credit spreads. Exposure is evaluated on the payment
dates, so no accrued coupon is left out (the time-integration error of the grid remains). Figures are illustrative: they show orders of magnitude,
...
- The Mexican regulatory chain: IFRS 13, NIF B-17 and C-10 (applicable through Anexo 33 CUB, CNBV),
  Formulario XVA and Circular 4/2012 (Banxico), CVA capital under the CUB, and BIS MAR 50 as the
  international reference; and why accounting CVA and regulatory CVA capital must not be conflated.

## Notes on the material

Market data is attributed to **Valmer** (TIIE de Fondeo curve and CDS) and to **Bloomberg** (ATM
swaption quotes used for the G2++ calibration), with a reference date of 24 March 2026. The 25%
recovery rate follows the market convention for emerging-market CDS. Portfolios
...
### Market Conventions and Notices

- Y. Iwashita. "Conventions for Single-Name Credit Default Swaps". OpenGamma Quantitative Research
  n. 17, 2013.
- CME Group. "Conversion Pricing for Cleared MXN 28D TIIE Swaps". Technical note, March 2024.
- MexDer y Asigna. "MexDer hace oficial el lanzamiento del Swap de TIIE de Fondeo". Boletín de prensa,
  26 de febrero de 2024.

### Regulatory Documents

- IASB. "IFRS 13: Fair Value Measurement".
- CNBV. "Disposiciones de carácter general aplicables a las instituciones de crédito" (CUB), Anexo 33,
  criterio A-2; and articles 2 Bis 98 a to 2 Bis 98 d (CVA capital).
- Banco de México. "Formulario XVA: Ayudas genéricas" (March 2022).
